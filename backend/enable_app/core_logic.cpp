...
{
	double score = 0.;
	const set<string> positive_words = { "good", "great","excellent" };
	const set<string> negative_words = { "bad", "sad", "terrible" };

	std::set<string> common_words;
	std::set_intersection(keywords.begin(), keywords.end(), positive_words.begin(), positive_words.end(), std::inserter(common_words, common_words.begin()));
	score += (0.5 * common_words.size());
	common_words.clear();
	
	std::set_intersection(keywords.begin(), keywords.end(), negative_words.begin(), negative_words.end(), std::inserter(common_words, common_words.begin()));
	score -= (0.5 * common_words.size());
